...
def FFT_inverse(paddedwidth, img_row):# w, c
    ...
    # if sub_array_num == 4 or sub_array_num == 8:
        ...
        return rg_real, rg_imag, ba_real, ba_imag
def HorizontalFFTAndSplitOneForTwo_npref(img):#
    h, w, _ = img.shape
    ...
    for col_index in tqdm(range(h)):
        ref_rg = np.fft.fft(img[col_index,:,0] + 1j * img[col_index,:,1])
        ref_ba = np.fft.fft(img[col_index,:,2] + 1j * img[col_index,:,3])
        # rg_real, rg_imag, ba_real, ba_imag = FFT(paddedWidth, img[col_index,:])
        rg_real = np.real(ref_rg)
        rg_imag = np.imag(ref_rg)
        ba_real = np.real(ref_ba)
        ba_imag = np.imag(ref_ba)
...
def LoadFromTwoForOne(paddedWidth, input_array):
    ...
    return output_array
def Vertical_MulFilter_UnitTest():
    horizontal_output = cv2.imread("horizontal_output.exr", cv2.IMREAD_UNCHANGED)
    converted = horizontal_output.copy()
    converted[:,:,0] = horizontal_output[:,:,2]
    converted[:,:,2] = horizontal_output[:,:,0]

    horizontal_output = converted
    ...
    # paddedhorizontaloutput = np.zeros((2048,4),dtype=np.float32)
    # paddedhorizontaloutput[:720,:] =  horizontal_output[:,col_index,:]
    # ComplexFFTResult_rg = np.fft.fft(paddedhorizontaloutput[:,0] + 1j * paddedhorizontaloutput[:,1])
    # ComplexFFTResult_ba = np.fft.fft(paddedhorizontaloutput[:,2] + 1j * paddedhorizontaloutput[:,3])
    ...
    # for i in range(8):
    #     print(ComplexFFTResult_rg[i * 256])
    # for i in range(8):
    #     print(ComplexFFTResult_ba[i * 256])
    # for i in range(8):
    #     print(ComplexKernel_rg[i * 256])
    # for i in range(8):
    #     print(ComplexKernel_ba[i * 256])

    # for i in range(8):
    #     print(Filtered_rg_complex[i * 256])
    # for i in range(8):
    #     print(Filtered_ba_complex[i * 256])
    ...
    Filtered[:,0] = np.real(Filtered_rg_complex) / kernelsum[0][0]
    Filtered[:,1] = np.imag(Filtered_rg_complex) / kernelsum[0][0]
    Filtered[:,2] = np.real(Filtered_ba_complex) / kernelsum[1][0]
    Filtered[:,3] = np.imag(Filtered_ba_complex) / kernelsum[1][0]
    # Filtered *= (1. / paddedLength)
    # rg_real_inv, rg_imag_inv, ba_real_inv, ba_imag_inv = FFT_inverse(paddedLength, Filtered)

    rg_inv = np.fft.ifft(Filtered[:,0]+ 1j*Filtered[:,1])
    ba_inv = np.fft.ifft(Filtered[:,2]+ 1j*Filtered[:,3])

    # rg_inv = rg_real_inv + 1j * rg_imag_inv
    # ba_inv = ba_real_inv + 1j * ba_imag_inv
    for i in range(8):
        print(rg_inv[i * 256])
    for i in range(8):
        print(ba_inv[i * 256])
...
def KernelFilter():
    kernel_img = cv2.imread("img/DefaultBloomKernel.exr", cv2.IMREAD_UNCHANGED)
    cooverted_kerrel = kernel_img.copy()
    cooverted_kerrel[:,:,0] = kernel_img[:,:,2]
    cooverted_kerrel[:,:,2] = kernel_img[:,:,0]

    horizontal_kererel_output = HorizontalFFTAndSplitOneForTwo(cooverted_kerrel)
    
    
    kernel_fft2d_output = horizontal_kererel_output.copy()
    for col_index in tqdm(range(horizontal_kererel_output.shape[1])):
        rg_real, rg_imag, ba_real, ba_imag = FFT(horizontal_kererel_output.shape[0], horizontal_kererel_output[:,col_index,:])
        kernel_fft2d_output[:,col_index,0] = rg_real
        kernel_fft2d_output[:,col_index,1] = rg_imag
        kernel_fft2d_output[:,col_index,2] = ba_real
        kernel_fft2d_output[:,col_index,3] = ba_imag
    cooverted_kerrel_fft = kernel_fft2d_output.copy()
    cooverted_kerrel_fft[:,:,0] = kernel_fft2d_output[:,:,2]
    cooverted_kerrel_fft[:,:,2] = kernel_fft2d_output[:,:,0]

    

    cv2.imwrite("kernel_2d_fft.exr", cooverted_kerrel_fft.astype(np.float32))
def KernelSum():
    kernel_img = cv2.imread("img/DefaultBloomKernel.exr", cv2.IMREAD_UNCHANGED)
    print(np.sum(kernel_img[:,:,0]))
    print(np.sum(kernel_img[:,:,1]))
    print(np.sum(kernel_img[:,:,2]))
    print(np.sum(kernel_img[:,:,3]))
...
def Input_kernel_convolution():
    input_img = cv2.imread("img/input_1280x720.exr", cv2.IMREAD_UNCHANGED)
    kernel_imag = cv2.imread("img/DefaultBloomKernel.exr", cv2.IMREAD_UNCHANGED)
    ...
    padded_img = np.zeros((2048, 2048, 4), dtype=np.float64)
    padded_img[:720, : 1280, :] = rgb_channel4
    ...
    kernel_imag[:,:,0] /= np.sum(kernel_imag[:,:,0])
    kernel_imag[:,:,1] /= np.sum(kernel_imag[:,:,1])
    kernel_imag[:,:,2] /= np.sum(kernel_imag[:,:,2])
    kernel_imag[:,:,3] /= np.sum(kernel_imag[:,:,3])

    filtered = [ convolution2d(padded_img[:,:,0], kernel_imag[:,:,0]), convolution2d(padded_img[:,:,1], kernel_imag[:,:,1]), convolution2d(padded_img[:,:,2], kernel_imag[:,:,2]), convolution2d(padded_img[:,:,3], kernel_imag[:,:,3]) ]

    # img_fft2d = [np.fft.fft2(padded_img[:,:,0]),np.fft.fft2(padded_img[:,:,1]),np.fft.fft2(padded_img[:,:,2]),np.fft.fft2(padded_img[:,:,3])]
    # kernel_fft2d = [np.fft.fft2(kernel_imag[:,:,0]),np.fft.fft2(kernel_imag[:,:,1]),np.fft.fft2(kernel_imag[:,:,2]),np.fft.fft2(kernel_imag[:,:,3])]

    # filtered_freq = [ img_fft2d[0] * kernel_fft2d[0], img_fft2d[1] * kernel_fft2d[1], img_fft2d[2] * kernel_fft2d[2], img_fft2d[3] * kernel_fft2d[3] ]
    # # filtered_freq = [ img_fft2d[0], img_fft2d[1], img_fft2d[2], img_fft2d[3] ]

    # filtered = [ np.fft.ifft2(filtered_freq[0]),np.fft.ifft2(filtered_freq[1]),np.fft.ifft2(filtered_freq[2]),np.fft.ifft2(filtered_freq[3]) ]

    result = input_img.copy()
    result[:,:,0] = np.real(filtered[0])[:720,:1280]
    result[:,:,1] = np.real(filtered[1])[:720,:1280]
    result[:,:,2] = np.real(filtered[2])[:720,:1280]
    result[:,:,3] = np.real(filtered[3])[:720,:1280]

    converted_result = result.copy()
    converted_result[:,:,0] = result[:,:,2]
    converted_result[:,:,2] = result[:,:,0]
